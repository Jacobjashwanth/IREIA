...
# 📌 STEP 3: Filter Only "For Sale" Properties
historical_data = historical_data[historical_data["Status"] == "for_sale"]

# 📌 STEP 4: Rename Columns & Fix Data Types
...
features = ["Year", "Month", "Crime Rate", "sentiment_score"]
target = "y"

# 📌 STEP 9: Train & Save Per-Property Models
...
for property_id in property_ids:
    property_subset = historical_data[historical_data["Property ID"] == property_id]

    if len(property_subset) < 2:
        print(f"⚠️ Skipping Property {property_id}: Not enough data.")
        continue

    X = property_subset[features]
    y = property_subset[target]
    ...
    print(f"\n📊 Model Performance for Property {property_id}:")
...
# 📌 FINAL GLOBAL MODEL — CORRECT SAVE
...
print("🎯 Final model training and saving completed successfully.")